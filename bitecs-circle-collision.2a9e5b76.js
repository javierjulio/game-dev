...
const movementSystem = (world)=>{
    ...
    for(let i = 0; i < ents.length; i++){
        ...
        if (Position.x[eid] - Shape.radius[eid] < 0) Velocity.x[eid] = Math.abs(Velocity.x[eid]);
        else if (Position.x[eid] + Shape.radius[eid] > canvasWidth) Velocity.x[eid] = -Math.abs(Velocity.x[eid]);
        if (Position.y[eid] - Shape.radius[eid] < 0) Velocity.y[eid] = Math.abs(Velocity.y[eid]);
        else if (Position.y[eid] + Shape.radius[eid] > canvasHeight) Velocity.y[eid] = -Math.abs(Velocity.y[eid]);
    }
    return world;
};
...
//# sourceMappingURL=bitecs-circle-collision.2a9e5b76.js.map
