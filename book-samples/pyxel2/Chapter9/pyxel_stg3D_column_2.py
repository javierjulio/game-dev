...
# 宇宙空間を進む背景に変更する
STAR_MAX = 200 # 星の数
st_x, st_y, = [999]*STAR_MAX, [0]*STAR_MAX # 座標を代入
st_vx, st_vy = [0]*STAR_MAX, [0]*STAR_MAX  # 各軸方向の速さを代入

def draw_3dbg(): # 疑似3Dの背景を描く
    for i in range(STAR_MAX):
        st_vx[i] *= 1.02 # ┬ 加速
        st_vy[i] *= 1.02 # ┘
        st_x[i] += st_vx[i] # ┬ 座標を変更
        st_y[i] += st_vy[i] # ┘
        if st_x[i] < 0 or WIDTH < st_x[i] or st_y[i] < 0 or HEIGHT < st_y[i]:
            st_x[i], st_y[i] = CX, CY # 消失点
            st_vx[i] = pyxel.rndf(-3.0, 3.0)
            st_vy[i] = pyxel.rndf(-2.0, 2.0)
        pyxel.pset(st_x[i], st_y[i], 7 + (i%2)*6) # 白か灰色で点を打つ
...
def draw_enemy(): # 敵機を表示する
    ...
    for i in range(EMY_MAX):
        if emy_action[i] != 0:
            order.append(emy_z[i] * 100 + i) # z座標x100 + 添え字
    order.sort(reverse=True) # データのソート
    for n in range(len(order)):
        i = order[n] % 100 # ソート後の添え字を取り出す
